...
pub(crate) fn resolve_rust_filesystem_path(
    ...
    if !is_fs_path {
        // Bare extensionless names (`include_str!("LICENSE")`) carry no path
        // signature at all. They are still tried against the candidate roots
        // below and only resolve when such a file actually exists — module
        // paths (`foo::bar`) are never filesystem candidates.
